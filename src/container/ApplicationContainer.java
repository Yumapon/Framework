...
public class ApplicationContainer {
	 ...
	 * @param instancePlace
	 */
	public ApplicationContainer(String beanConfigFileName, String actionConfigFileName, String businessLogicConfigName){
	...
	public Object generator(String instanceName) {
		...
		try {
			...
			//処理内容
			System.out.println(DisplayText.BEANCREATECOMPLETE);
		} catch (ClassNotFoundException e) {
			//ログ発生箇所
			System.out.print(Thread.currentThread().getStackTrace()[1].getClassName() + ":");
			//例外内容
			System.out.println(DisplayText.CLASSPASSNOTFOUND + "（" + instancePlace + "が見つかりません。）");
			//スタックトレース
			e.printStackTrace();
		}catch (Exception e) {
			//スタックトレース
		...
		public Object generator() {
			...
			try {
				...
				if(instancePlace == null) {
					System.out.println(DisplayText.NOTDESIGNATIONCLASSPASS);
				}else {
					System.out.println(DisplayText.CLASSPASSNOTFOUND + "（" + instancePlace + "が見つかりません。）");
				}
			}catch (Exception e) {
